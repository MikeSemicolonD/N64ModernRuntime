...
namespace N64Recomp {
    ...
    public:
        ...
        // Materialize the link register ($ra = return address) for a branch-and-link.
        // Required for code that reads $ra as data (PC-relative addressing, e.g. the
        // `bltzal $zero`/`bgezal $zero` table-lookup idiom in the MusyX voice filter).
        // Default no-op for backends that don't materialize $ra.
        virtual void emit_link_address(int reg, uint32_t address) const { (void)reg; (void)address; }
    ...
    public:
        ...
        void emit_link_address(int reg, uint32_t address) const final;
