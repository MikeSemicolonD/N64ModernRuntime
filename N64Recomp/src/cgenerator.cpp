...
void N64Recomp::CGenerator::emit_link_address(int reg, uint32_t address) const {
    // Sign-extend the 32-bit link address into the 64-bit gpr to match MIPS semantics.
    fmt::print(output_file, "{} = (gpr)(int32_t)0x{:08X}u;\n", gpr_to_string(reg), address);
}
